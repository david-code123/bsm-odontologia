const WA_LINK = 'https://wa.me/5562993557723?text=Olá!%20Gostaria%20de%20agendar%20uma%20consulta%20na%20BSM%20Odontologia.'

export default function Hero() {
  return (
    <section id="inicio" className="hero" aria-label="Seção principal">
      <div className="hero-bg" aria-hidden="true" />
      <div className="hero-grid" aria-hidden="true" />

      <div className="container">
        <div className="hero-inner">
          <div className="hero-content reveal-left">
            <div className="hero-badge" aria-label="Avaliação">
              <span className="hero-badge-dot" />
              ★ 5,0 no Google · +500 Pacientes Satisfeitos
            </div>

            <h1 className="hero-title">
              Transformando Sorrisos com{' '}
              <span>Tecnologia e Excelência</span>
            </h1>

            <p className="hero-subtitle">
              Tratamentos odontológicos modernos para devolver sua autoestima
              e qualidade de vida. Agende sua consulta em Goiânia/GO.
            </p>

            <div className="hero-actions">
              <a
                href={WA_LINK}
                target="_blank"
                rel="noopener noreferrer"
                className="btn btn-whatsapp"
                aria-label="Agendar consulta pelo WhatsApp"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                Agendar Consulta
              </a>
              <a href="#tratamentos" className="btn btn-outline">
                Conhecer Tratamentos
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                  <path d="M5 12h14M12 5l7 7-7 7"/>
                </svg>
              </a>
            </div>

            <div className="hero-stats" role="list" aria-label="Estatísticas da clínica">
              <div role="listitem">
                <div className="hero-stat-number">500<span>+</span></div>
                <div className="hero-stat-label">Pacientes Atendidos</div>
              </div>
              <div role="listitem">
                <div className="hero-stat-number">5<span>.0</span></div>
                <div className="hero-stat-label">Estrelas no Google</div>
              </div>
              <div role="listitem">
                <div className="hero-stat-number">8<span>+</span></div>
                <div className="hero-stat-label">Especialidades</div>
              </div>
            </div>
          </div>

          <div className="hero-visual reveal-right">
            <div className="hero-image-wrapper">
              {/* ─────────────────────────────────────────────────────────────
                  FOTO HERO — substituir pelo arquivo real da clínica/equipe
                  Tamanho ideal: 800×1000px  |  Formato: JPG ou WebP
                  Exemplo de uso depois:
                  <img src="/images/hero-consultorio.jpg" alt="Consultório BSM" style={{ width:'100%', height:'100%', objectFit:'cover', borderRadius:32 }} />
              ───────────────────────────────────────────────────────────── */}
              <div className="hero-image-main">
                <img
                  src="/images/clinic-entrada.jpg"
                  alt="Fachada da BSM Odontologia e Estética Avançada em Goiânia"
                  style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 32 }}
                  loading="eager"
                />
              </div>

              <div className="hero-card-float hero-card-rating" role="img" aria-label="Avaliação 5 estrelas no Google">
                <div className="hero-card-rating-stars" aria-hidden="true">
                  {'★★★★★'.split('').map((s, i) => <span key={i}>{s}</span>)}
                </div>
                <div>
                  <div className="hero-card-rating-text">5,0 Estrelas</div>
                  <div className="hero-card-rating-sub">Google Reviews</div>
                </div>
              </div>

              <div className="hero-card-float hero-card-patients" role="img" aria-label="Mais de 500 pacientes atendidos">
                <div className="hero-card-patients-num">500+</div>
                <div className="hero-card-patients-label">Pacientes<br/>Satisfeitos</div>
              </div>
            </div>
          </div>
        </div>
      </div>

    </section>
  )
}
